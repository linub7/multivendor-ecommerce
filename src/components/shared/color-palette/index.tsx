...
interface Props {
  ...
  imgColors?: { color: string }[]; // List of selected colors from from
...
const ColorPalette = (props: Props) => {
  ...
  const handleAddProductColor = (color: string) => {
    ...
    const newColors = currentColorsData.filter((item) => item.color !== '');

    // Add new color to colorsData
    setImgColors([...newColors, { color }]);
  };
    ...
    <div className="pt-10 w-[320px] h-[160px] rounded-b-md overflow-hidden">
      ...
      <div className="w-[320px] h-[180px] rounded-md perspective-1000">
        ...
        <div className="w-full h-[180px] absolute bottom-0 !flex items-center justify-center">
          {/* Map over colors to display color */}
          {extractedColors?.map((color: string, index: number) => (
            <CustomColor
              key={index}
              color={color}
