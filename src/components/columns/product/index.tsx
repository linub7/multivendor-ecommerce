...
export const ProductColumns: ColumnDef<StoreProductType>[] = [
  {
    accessorKey: 'image',
    header: '',
    cell: ({ row }) => {
            ...
            {row.original.variants.map((variant) => (
              <div key={variant.id} className="flex flex-col gap-y-2 group">
                <div className="relative cursor-pointer">
                  <Image
                    src={variant.productVariantImages[0].url}
                  ...
                  >
                    <div className="w-full h-full absolute top-0 left-0 bottom-0 right-0 z-10 rounded-sm bg-black/50 transition-all duration-150 hidden group-hover:block">
                  ...
                  <div className="flex mt-2 gap-2 p-1">
                    ...
                    <div className="w-7 flex flex-col gap-2 rounded-md">
                      {variant.productVariantColors.map((color) => (
                        <span
                          key={color.id}
